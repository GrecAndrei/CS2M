...
{
    public class FinanceSyncCommandHandler : CommandHandler<FinanceSyncCommand>
    {
        ...
        public FinanceSyncCommandHandler()
        {
        }

        protected override void Handle(FinanceSyncCommand command)
        {
        ...
        private static void HandleOnServer(FinanceSyncCommand command)
        {
            if (!command.RequestOnly) return;

            if (!MarkNonce(command.TransactionNonce, ProcessedRequestNonces, ProcessedRequestNonceOrder, RequestNonceLock))
            {
                return;
            }

            bool applied = FinanceSyncService.TryApplyFinance(command);
            if (!applied) return;
            ...
            CS2M.API.CooperativeActivityRegistry.RegisterActivity(
                ...
                "Adjusted city tax codes and budgets",
                0f, 0f, 0f
            );
        }

        private static void HandleOnClient(FinanceSyncCommand command)
        {
            if (command.RequestOnly) return;

            if (!MarkNonce(command.TransactionNonce, ProcessedReplicationNonces, ProcessedReplicationNonceOrder, ReplicationNonceLock))
            {
                return;
            }
